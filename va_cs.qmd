...
#ANALYSIS TOGGLE
######################################
valset  <- 1 #set valset to 0 for first iteration; 1 for updated data iteration
...
tabout |> 
  ...
  datatable(extensions = 'Buttons', rownames=FALSE, colnames=c('Program', 'Number of Staff', 'Overall Recidivism Rate'), 
                          options=list(pageLength=10, dom='Bt',
  buttons = list(
          list(extend = "csv", text = "Download Table View", filename = "view_overall",
               exportOptions = list(
...
datatable(tabout2, extensions = 'Buttons', rownames=FALSE, colnames=c('Year', 'Program', 'Number of Staff', 'Recidivism Rate'), 
...
```{webr-r}
#| autorun: false
ALL.BY  <-                     #F=overall years; T=by year
...
rrfinal<- rr + geom_hline(yintercept=w[1], linetype = "dashed", color = hlinew1, size = 1) +
...
```{r dataviz2print, echo=FALSE}
suppressMessages(download_this(rrfinal, button_label = "Download Plot", class = "button_large", output_name = paste0("recidivism_rate_",date1,"_to_",date2)))
```

## More Cool Data Viz!

Let's prepare our data to do some really fun data viz! First let's combine everything together in a way that allows us to produce visualizations by year or overall.
...
Now that we have our data in the right format, shown in our sample of the data above, what are some more engaging ways we can plot recidivism rates for leadership and our stakeholders overall for these programs?
...
This exploratory document has been really useful! But what if we want to get all of this into a single report for your Director in a format they can actually digest?
