...
roster.prep <- data.frame(
  ...
  mutate(programs  = substr(programs.temp,1,nchar(programs.temp)-2),
         ...
         datestart = case_when(row_number() == sizen-3  ~ as.Date(paste0('2001/06/05')), 
                               row_number() == sizen-20 ~ as.Date(paste0('1991/08/22')),
                            TRUE ~ datestart),
...
roster.update <- data.frame(
  ...
  datestart       = as.Date("2024-01-01") + round(rnorm(sizen, mean = 0, sd = 120))
  ) |>
  mutate(programs = substr(programs.temp,1,nchar(programs.temp)-2),
         ret  = as.numeric(substr(programs.temp,nchar(programs.temp),nchar(programs.temp)))
         ) |>
...
#ANALYSIS TOGGLE
######################################
roster  <- roster.update        #options: roster; roster.update
date1   <- date3         #options: date1 (within roster); date3 (within roster.update)
date2   <- date4         #options: date2 (within roster); date4 (within roster.update)
...
```{r dataviz6, warning=FALSE, message=FALSE, eval=!ALL.BY}
#| code-fold: true
